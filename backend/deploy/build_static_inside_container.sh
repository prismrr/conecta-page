...
OUTPUT_DIR="${BUILD_STATIC_OUTPUT_DIR:-$ROOT_DIR/.deploy/dist}"
NUXT_OUTPUT_DIR="$ROOT_DIR/nuxt-app/.output/public"
BUILD_ENV="${BUILD_STATIC_BUILD_ENV:-${DEPLOY_ENV:-unknown}}"

rm -rf "$OUTPUT_DIR"
mkdir -p "$OUTPUT_DIR"

echo "[deploy] building Nuxt SSG artifact in container"
...
cat > "$OUTPUT_DIR/RELEASE_MANIFEST.json" <<EOF
{
  ...
  "builtAt": "${BUILD_TIME}",
  "buildSource": "backend/deploy/build_static_inside_container.sh",
